...
`install.sh` 和 `run.sh` 在启动后始终校验所下载二进制的 SHA-256。若系统已有 `cosign`，它们还会自动验证二进制签名。若没有 `cosign`，交互终端会明确提示用户输入 `continue` 才能退回到仅校验 SHA-256；非交互运行默认停止，除非操作者在理解同一 Release 中校验和不能认证发布者的前提下显式设置 `VPS_SCOPE_ALLOW_UNSIGNED=1`。显式设置 `VPS_SCOPE_VERSION=v1.0.0` 或使用安装参数 `--version v1.0.0` 时，证书身份必须精确匹配这个标签；使用 `latest` 时，身份仍被限制在本仓库、`release.yml` 和规范语义版本标签范围内。
