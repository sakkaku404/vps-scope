...
func overallVerdictFor(r model.Report, locale string) overallVerdict {
	...
	if confirmed == 0 && gaps == 0 {
		return overallVerdict{choose(locale, "本次未发现明确风险", "No confirmed risks in this run"), choose(locale, "这不等于服务器绝对安全；它表示本次可读取证据未触发风险判断。", "This is not proof of an uncompromised host; available evidence did not trigger a risk finding.")}
	}
	if confirmed == 0 {
...
func summarizeActions(r model.Report, locale string) actionSummary {
	...
	for _, finding := range r.Findings {
		...
		item := actionItem{Localized: localize(finding, locale), Verdict: verdictForFinding(finding, locale)}
...
func verdictForFinding(f model.Finding, locale string) string {
	...
	switch f.ID {
	case "SSH-001":
		return choose(locale, "明确风险：SSH 密码认证已经生效。", "Confirmed risk: SSH password authentication is effective.")
	case "SSH-002":
		return choose(locale, "明确风险：root 可以直接通过 SSH 登录。", "Confirmed risk: root can log in directly through SSH.")
	case "WORK-002":
		...
		case defaultPath && plaintext:
			return choose(locale, "明确风险：管理面从公网明文开放，并使用根或默认路径；路径隐藏不能替代访问控制。", "Confirmed risk: the management panel is publicly reachable over plaintext at a root/default path; path obscurity is not access control.")
		case defaultPath:
			return choose(locale, "明确风险：管理面可从公网访问，并使用根或默认路径；应限制访问来源。", "Confirmed risk: the management panel is publicly reachable at a root/default path; restrict its sources.")
		case plaintext:
			return choose(locale, "明确风险：管理面从公网明文开放；应启用 TLS 并限制访问来源。", "Confirmed risk: the management panel is publicly reachable over plaintext; enable TLS and restrict its sources.")
		default:
			return choose(locale, "明确风险：管理面可从公网访问，应限制访问来源。", "Confirmed risk: a management panel is reachable from the public internet; restrict its sources.")
		}
	case "WORK-005":
		return choose(locale, "明确风险：代理控制 API 可从公网访问，应改为回环监听或限制访问来源。", "Confirmed risk: a proxy control API is publicly reachable; bind it to loopback or restrict its sources.")
	case "WORK-012":
	...
	case "WORK-009":
		if blocked {
			return choose(locale, "可用性问题：已配置的代理入口被主机防火墙阻断。", "Availability issue: a configured proxy ingress is blocked by the host firewall.")
		}
		return choose(locale, "需要核对：配置、运行监听或防火墙证据不一致。", "Review needed: configuration, live listener, or firewall evidence does not agree.")
	case "TLS-001":
		return choose(locale, "可用性问题：证书已过期、即将过期或续期证据不足。", "Availability issue: the certificate is expired, near expiry, or renewal evidence is missing.")
	case "FW-002":
	...
	if f.Status == model.Unknown {
		return choose(locale, "证据不足：本次审计不能确认这一项安全。", "Evidence gap: this audit cannot confirm that this item is safe.")
	}
	return choose(locale, "需要处理：请结合下面的证据和建议确认。", "Action needed: confirm using the evidence and suggestion below.")
}
