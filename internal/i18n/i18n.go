...
var Rules = map[string]Rule{
	...
	"WORK-013":    newRule("反向代理暴露链", "Reverse-proxy exposure chain", "公网前端、反向代理和本地管理面或代理核心之间的关系不一致，可能导致服务中断或后端绕过。", "Disagreement between a public frontend, reverse proxy, and local management or proxy backend can cause outages or backend bypass.", "核对前端和后端监听地址、端口、进程与主机防火墙；不需要直连的后端应绑定回环或容器私网。", "Review frontend and backend listeners, ports, processes, and host firewall; bind backends that do not require direct access to loopback or a container-private network."),
	"WORK-012":    newRule("代理面板角色与运行态一致性", "Proxy panel roles and runtime consistency", "管理、订阅和代理入口应有明确角色，面板数据库、生成配置和实际监听不一致可能表示遗留配置、未生效变更或错误暴露。", "Management, subscription, and proxy ingress need explicit roles; disagreement between the panel database, generated configuration, and live listeners can indicate stale state, unapplied changes, or unintended exposure.", "核对角色冲突、已启用但未监听的入口和未分类面板端口；不要把订阅端口误判为管理端口。", "Review role collisions, enabled-but-not-listening ingress, and unclassified panel-owned ports; do not mistake subscription endpoints for management ports."),
