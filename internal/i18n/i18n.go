...
func normalizeExtraCatalogs() {
	...
	for key, value := range map[string]string{
		...
		"native workload self-test: disabled by default; audit --native-self-test executes local workload code after trust checks": "самопроверка локальных программ отключена по умолчанию; audit --native-self-test выполняет локальный код после проверки доверия",
	...
	for key, value := range map[string]string{
		...
		"native workload self-test: disabled by default; audit --native-self-test executes local workload code after trust checks": "خودآزمایی برنامه محلی به‌طور پیش‌فرض غیرفعال است؛ audit --native-self-test پس از بررسی اعتماد کد محلی را اجرا می‌کند",
