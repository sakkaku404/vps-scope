...
var Categories = map[string]Text{
	...
	"auth":        {"认证、Fail2ban 与 sudo 日志", "Authentication, Fail2ban, and sudo logs"},
...
var Rules = map[string]Rule{
	...
	"AUTH-003":    newRule("Fail2ban 运行状态", "Fail2ban operational status", "只安装但未运行或没有 sshd jail 不会产生预期防护。", "An installed but inactive service or missing sshd jail does not provide expected protection.", "核对服务、jail 和实际日志后端。", "Review the service, jails, and actual log backend."),
	...
	"WORK-002":    newRule("S-UI 管理面暴露", "S-UI management-plane exposure", "公网通配监听且允许任意来源访问的管理面会扩大凭据攻击和漏洞利用面。", "A management panel bound publicly and allowed from any source expands credential and exploit exposure.", "将管理面限制到回环、VPN 或可信来源地址；订阅和代理入站应与管理端口分开处理。", "Restrict the management panel to loopback, a VPN, or trusted source addresses; treat subscription and proxy ingress separately."),
