...
func normalizeExtraCatalogs() {
	...
	for key, value := range map[string]string{
		...
		"Host":                                 "Сервер",
		"VPS Scope doctor":                     "VPS Scope — диагностика среды",
		"OS":                                   "Система",
		"Architecture":                         "Архитектура",
		"Full audit supported":                 "Полный аудит поддерживается",
		"Profile":                              "Профиль",
		"Top finding":                          "Главный результат",
		"no RISK; review INFO as needed":       "RISK нет; при необходимости просмотрите INFO",
		"support bundle: %s (%d files total: %d content files plus manifest.json)\n":                                                    "пакет поддержки: %s (всего файлов: %d; файлов данных: %d и manifest.json)\n",
		"Review every file before sharing. Raw configuration, databases, credentials, keys, UUIDs and host addresses are not included.": "Перед отправкой проверьте каждый файл. Исходные конфигурации, базы данных, учётные данные, ключи, UUID и адреса узла не включаются.",
		"report schema=%s tool=%s findings=%d\n":                                                                                        "схема отчёта=%s версия инструмента=%s проверок=%d\n",
		"PASS report structure and semantic contract are valid":                                                                         "PASS структура отчёта и семантический контракт корректны",
		"Completed":        "Выполнено",
	...
	for key, value := range map[string]string{
		...
		"VPS Scope doctor":                     "VPS Scope — بررسی محیط",
		"OS":                                   "سیستم",
		"Architecture":                         "معماری",
		"Full audit supported":                 "ممیزی کامل پشتیبانی می\u200cشود",
		"Profile":                              "پروفایل",
		"Top finding":                          "مهم\u200cترین نتیجه",
		"no RISK; review INFO as needed":       "مورد RISK ثبت نشده است؛ در صورت نیاز موارد INFO را بررسی کنید",
		"support bundle: %s (%d files total: %d content files plus manifest.json)\n":                                                    "بسته پشتیبانی: %s (در مجموع %d فایل: %d فایل محتوا و manifest.json)\n",
		"Review every file before sharing. Raw configuration, databases, credentials, keys, UUIDs and host addresses are not included.": "پیش از اشتراک\u200cگذاری همه فایل\u200cها را بررسی کنید. پیکربندی خام، پایگاه داده، اطلاعات ورود، کلیدها، UUID و نشانی میزبان در بسته قرار نمی\u200cگیرند.",
		"report schema=%s tool=%s findings=%d\n":                                                                                        "schema گزارش=%s نسخه ابزار=%s تعداد بررسی=%d\n",
		"PASS report structure and semantic contract are valid":                                                                         "PASS ساختار گزارش و قرارداد معنایی معتبر است",
...
var Rules = map[string]Rule{
	"WORK-017":    newRule("离线安全公告匹配", "Offline security advisory matching", "已知受影响版本可能包含已公开的安全缺陷；离线数据库只能覆盖项目随版本发布时已收录的公告。", "Known affected versions may contain publicly disclosed security flaws; the offline database only covers advisories included when this release was built.", "核对公告链接和实际版本，优先升级到官方修复版本；若公告数据库过旧，请先更新 VPS Scope 后复查。", "Review the advisory and detected version, then upgrade to the vendor's fixed release; update VPS Scope before relying on a stale advisory database."),
	"WORK-016":    newRule("代理出口路径与 DNS", "Proxy egress path and DNS", "代理流量、系统默认路由和 DNS 出口不符合部署预期时，可能导致泄漏、绕路或不可用。", "Proxy traffic, default routes, and DNS egress that differ from the intended deployment can cause leaks, routing surprises, or outages.", "用策略文件声明预期出口接口、IPv4/IPv6 路径和 DNS 模式，并结合客户端侧观察复核。", "Declare expected egress interfaces, IPv4/IPv6 paths, and DNS mode in a policy file, then confirm them with client-side observations."),
	"WORK-015":    newRule("部署策略一致性", "Deployment policy conformance", "没有显式部署意图时，工具只能从运行状态推断端口用途；策略文件可把预期暴露面与实际证据进行确定性比较。", "Without explicit deployment intent, the tool can only infer endpoint roles; a policy file enables deterministic comparison between expected exposure and observed evidence.", "为管理面、订阅端点、代理入口和控制 API 声明角色、来源限制、TLS 与路径要求，并在变更后重新审计。", "Declare roles, source restrictions, TLS, and path requirements for management, subscription, proxy, and control endpoints, then re-audit after changes."),
	...
	"NET-004":     newRule("外部可达性观察", "External reachability observation", "主机自身的监听和防火墙状态不能证明互联网侧的真实可达性；第二观察点可发现云防火墙、路由和地址族差异。", "Local listeners and firewall state cannot prove real Internet reachability; a second vantage can expose cloud-firewall, routing, and address-family differences.", "在另一台受控主机生成观察结果并导入；TCP 结果应与部署策略一致，UDP 仍需协议客户端验证。", "Run the observation from another controlled host and import it; TCP results should match deployment policy, while UDP still requires a protocol-aware client test."),
